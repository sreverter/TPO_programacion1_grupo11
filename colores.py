color = {
    #  Reset / formato base
    "reset": "\033[0m",
    "negrita": "\033[1m",
    "subrayado": "\033[4m",

    #  Colores estándar
    "negro": "\033[30m",
    "rojo": "\033[31m",
    "verde": "\033[32m",
    "amarillo": "\033[33m",
    "azul": "\033[34m",
    "violeta": "\033[35m",
    "cian": "\033[36m",
    "blanco": "\033[37m",

    #  Colores brillantes (high intensity)
    "negro_brillante": "\033[90m",
    "rojo_brillante": "\033[91m",
    "verde_brillante": "\033[92m",
    "amarillo_brillante": "\033[93m",
    "azul_brillante": "\033[94m",
    "violeta_brillante": "\033[95m",
    "cian_brillante": "\033[96m",
    "blanco_brillante": "\033[97m",

    #  Fondos
    "fondo_negro": "\033[40m",
    "fondo_rojo": "\033[41m",
    "fondo_verde": "\033[42m",
    "fondo_amarillo": "\033[43m",
    "fondo_azul": "\033[44m",
    "fondo_violeta": "\033[45m",
    "fondo_cian": "\033[46m",
    "fondo_blanco": "\033[47m",

    #  Fondos brillantes
    "fondo_negro_brillante": "\033[100m",
    "fondo_rojo_brillante": "\033[101m",
    "fondo_verde_brillante": "\033[102m",
    "fondo_amarillo_brillante": "\033[103m",
    "fondo_azul_brillante": "\033[104m",
    "fondo_violeta_brillante": "\033[105m",
    "fondo_cian_brillante": "\033[106m",
    "fondo_blanco_brillante": "\033[107m",

    #Colores semánticos para tu sistema
    "titulo": "\033[1;94m",           # azul brillante + negrita
    "subtitulo": "\033[95m",          # violeta
    "opcion": "\033[36m",             # cian
    "input": "\033[1;36m",            # cian negrita
    "exito": "\033[1;92m",            # verde brillante + negrita
    "error": "\033[1;91m",            # rojo brillante + negrita
    "advertencia": "\033[1;93m",      # amarillo brillante + negrita
    "info": "\033[94m",               # azul brillante
}